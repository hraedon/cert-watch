...
docker logs cert-watch   # the one-time admin password is printed here
```
...
`cert-watch.service` unit from `deploy/systemd/`. Configure it with an
environment file referenced by the unit. Put a reverse proxy such as nginx or
...
- creates a shared Python runtime and virtual environment under
  `C:\ProgramData\cert-watch`;
- generates persistent signing keys;
- sets the application pool to `AlwaysRunning` and the application to
  `preloadEnabled`;
- verifies that IIS's Application Initialization feature is installed, and
  fails loudly if it is missing.

Those last three matter. Without them IIS starts cert-watch only when the
...
| Listening on a routable address, or on loopback behind a proxy (`CERT_WATCH_TRUST_PROXY=1`) | Creates an `admin` account with a random password, writes it to `<data dir>/initial-admin-password` and prints it to the log. |

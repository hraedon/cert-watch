...
| JSON API bodies are parsed only up to 256 KiB and 64 levels of nesting, checked on the decoded value (so the body's encoding doesn't matter), and never by relying on `RecursionError`, which CPython 3.14.7 no longer raises for deep input. Over either bound is `400` | `routes/api/_shared.py` (`json_body`) | `tests/test_security_hardening.py::test_json_nesting_is_bounded_explicitly_not_by_recursion`, `::test_json_body_size_is_capped_below_the_request_limit`, `::test_json_writes_reject_extreme_nesting_without_500` |

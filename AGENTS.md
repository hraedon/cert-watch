...
Unresolved backlog (as of 2026-06-02): **BC-020** (host-level notes — feature, medium), **BC-021** (progressive enhancement for dashboard — improvement, low), **BC-093** (enforce mypy strict in CI — improvement, low), **BC-094** (decompose queries.py by concern — improvement, low), **BC-095** (deduplicate SSRF pre-check logic — improvement, low), **BC-096** (decouple test monkeypatching from route-level import names — improvement, low), **BC-097** (Discover page sync CT reconciliation perf — improvement, medium), **BC-098** (detail page losing panel cards — defect, medium), **BC-099** (mis-issuance detection stubbed out — feature, medium), **BC-100** (fleet posture hardcoded CA detection — improvement, low), **BC-101** (trend charts need monthly bucketing — defect, medium). Resolved: BC-071 (OAuth userinfo nonce binding — nonce generation + verification implemented), BC-073 (grouped dashboard full-load — SQL GROUP BY pagination), BC-075 (CSP script-src unsafe-inline — all inline handlers converted, nonce CSP active), BC-081 (session revocation via per-user version counter), BC-082 (kv_store plaintext secrets — Fernet encryption at rest), BC-083 (secure-by-default auth), BC-084 (A+ non-443 posture), BC-085 (3.13 container), BC-086 (read-only vs read-write role tier), BC-087 (webhook/test missing write gate), BC-088 (create alert-group ignored webhook URL validation), BC-089 (CSV export duplicate columns), BC-090 (BC-083 check regressed IIS startup; entrypoint now the single source of truth for the bind host), BC-091 (mypy strict — dup of BC-093), BC-092 (queries.py decomposition — dup of BC-094). Resolved breadcrumbs live in the DB as history.

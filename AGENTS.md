...
- **`database.py`** — Repository pattern (`CertificateRepository`, `AlertRepository`, `SqliteHostRepository`). `replace_scanned()` does atomic delete+insert in one transaction. `init_schema()` is idempotent with column migration. Migration 0005 adds composite indexes `idx_cert_host_port_leaf` and `idx_scan_history_host_port_ts`. Migration 0007 adds `kv_store` table for setup wizard and boot config. `kv_get()`, `kv_set()`, `kv_all()` helpers in `queries.py`.

...
- **First finding from seeding:** WI-073 (high) — posture marks all public-trust chains "incomplete" on Windows because `_SYSTEM_CA_BUNDLE_PATHS` (cert_chain.py) is Linux-only and ignores the installed `certifi` bundle; every LE/AD cert grades B instead of A.

...
Version comes from `git describe --tags --abbrev=0` at Docker build time, injected via `GIT_TAG` and `GIT_COMMIT` build args. The Dockerfile writes these to `src/cert_watch/_version.txt`, and `__init__.py` reads it at import time (stripping any `v` prefix). The UI shows `v{version} ({commit})` in the header. `/healthz` deliberately does **not** — its body is `{"status": "ok"}` and nothing else, because the liveness probe is unauthenticated and build metadata is disclosure (BC-029 H). `/readyz` reports readiness checks only, also without build metadata; read the running version from the UI header or the OpenAPI document's `info.version`.
...
2. **Read the scaffold.** `src/cert_watch/` — `app.py` (FastAPI app factory + lifespan), `routes/` (HTTP handlers), `middleware.py` (middleware wiring; the layers live in `security/` and `auth/request_context.py`, route guards in `auth/guards.py`), `templates/`, `static/`, plus feature modules: `certificate_model.py`, `cert_chain.py`, `scan.py`, `upload.py`, `alerts.py`, `scheduler.py`, `posture.py`, `config.py`, `attention.py` (home-page attention-queue assembly). The `auth/` package and `database/` package hold the two largest concerns (see Architecture notes). **IA note (2026-08-30, branch `redesign/attention-home`):** `/` is Home (attention queue + expiry horizon), the inventory table lives at `/browse`; legacy `/` query-param bookmarks 307-redirect to `/browse`. Notes are host-scoped only (migration 0031 merges `certificates.notes`, retaining the deprecated column only for unmatched notes; see UI-INVENTORY V1/V2).
3. **Note the deploy story.** See `deploy/` (k8s + Argo CD, docker compose, systemd, IIS). Argo CD watches `deploy/k8s/`; CI bumps the image tag there on every merge to `main`. Do not commit changes to `deploy/k8s/kustomization.yaml` in feature PRs. Windows/IIS hosting (`deploy/iis/`, `scripts/install-windows.ps1`) fronts uvicorn via HttpPlatformHandler or an ARR reverse proxy; the app is cross-platform (the only OS-specific bit is the `CERT_WATCH_DATA_DIR` default — see `config._default_data_dir`). IIS installs **must** have both app-pool `AlwaysRunning` **and** application `preloadEnabled=true` — the installer sets both; without preload the scheduler dies silently at first reboot (WI-140).
...
- **Route-level auth via dependencies — one guard per route.** Every route declares exactly one guard from `auth/guards.py`: `require_auth` / `require_admin` / `admin_page_guard` for reads; `write_guard` / `admin_write_guard` (JSON) or `write_form_guard` / `admin_form_guard` / `admin_settings_form(...)` (`settings_tab_form(tab)`) (HTML forms) for mutations. Mutation guards always include CSRF; `tests/test_route_guards.py` fails any mutating route without exactly one, and `tests/test_authz_characterization.py` pins every route × principal outcome (regenerate its golden file only for a deliberate authorization change). Never hand-roll session/CSRF checks in handlers. Tag scope for notes/tags/ownership is enforced by the services (pass `auth=acting_auth(request)`), not the route. `require_auth` returns `""` (not 401) under `NoAuthProvider` so the "auth off = open" contract holds. Rate-limited API routes use `Depends(rate_limit("<prefix>", max, window))`; only form-POST/redirect routes (`/hosts`, `/login`, `/upload`) keep a manual `check_rate_limit` (a dependency can't return a redirect).
...
- **`CERT_WATCH_HISTORY_RETENTION_DAYS`** (default `365`), **`CERT_WATCH_ALERT_RETENTION_DAYS`** (default `90`), **`CERT_WATCH_AUDIT_RETENTION_DAYS`** (default `90`) — days of history/alerts/audit to keep; purged at startup + daily; `0` disables. Alert retention applies to *delivered* alerts; undelivered ones (pending/failed, `sent_at IS NULL`) are kept `UNDELIVERED_RETENTION_MULTIPLIER` (4×) longer so a delivery outage leaves a record (#39; `database/pagination.py`).
...
Plan status:
- **Plan 018** (auth/data consolidation): Phase A (A1/A2/A3) **done**; B2 (dashboard queries) **done**; **B1 (SecurityContext + create_app) — core done**: signing material is an immutable `SecurityContext` on `app.state` (`cert_watch.security`), `create_app(security=, auth_provider=, settings=)` injects deps, the session/CSRF request path reads `app.state.security`, and the test harness (`conftest`, `reload_app`) is reload-free. The remaining tail is that the lifespan still seeds module globals to back the OAuth-state signing path (no request available). The test harness no longer uses inline `importlib.reload`.
...
- **Plan 050** (alert routing e2e): recipient resolution and orphan notices shipped earlier; the TLS/AUTH SMTP + HTTP receipt matrix and offline `routing-report` diagnostic are now implemented. `ci.yml` explicitly runs the root-level receipt integration modules. Real-estate backup acquisition and human review remain operator-paced; synthetic receipts do not qualify production routing. See `docs/alert-routing-evidence.md`.
...
- **`security/__init__.py` + `create_app`** — `SecurityContext(signing_key, csrf_secret)` is an immutable dataclass resolved once in the lifespan (from `config.resolve_or_persist_secret`, persisted to `data_dir/.auth_secret`) and stored on `app.state.security`. `app.create_app(*, security=None, auth_provider=None, settings=None)` is the factory; `None` deps are resolved from env in the lifespan (production / the module-level `app`), explicit deps are injected (tests). Session signing (`auth.session`) and CSRF (`security.csrf`) functions take an optional `security` param; the request path passes `_request_security(request)` (reads `app.state.security`), and the module-level globals remain only as an import-time fallback for request-less paths (OAuth state signing, direct unit-test calls).
...
- **`middleware.py` + `security/` + `auth/request_context.py` + `auth/guards.py`** (split from one module in plan 057 W6) — `security/csrf.py`: CSRF (double-submit, header/form only — BC-070) and the `cw_sid` cookie; `security/ratelimit.py`: SQLite-backed rate limiting (`check_rate_limit`, `_init_rate_db`), proxy-aware `_extract_client_ip` (`CERT_WATCH_TRUST_PROXY`), `rate_limit(...)`; `auth/request_context.py`: session / API-key authentication, `check_metrics_token`, public paths, `auth_middleware`; `auth/guards.py`: the guard family. `CSPNonceMiddleware` (pure ASGI) issues the per-request CSP nonce into `scope['state']`; `security_headers_middleware` builds the response CSP via `_build_csp(nonce)` (BC-075 — nonce CSP active). `set_csrf_secret()` for lifespan-time key rotation. `setup_redirect_middleware` redirects HTML pages to `/setup` on fresh installs (never `/api/*` or public paths).
...
- **`digest.py`** — Weekly renewal digest (Plan 048 WI-3.1). `build_renewal_digest()` queries event_log for `cert_renewed` and `renewal_overdue` events, groups by owner. Zero-activity periods produce nothing. `SchedulerContext` persists successful expiry/renewal digest ISO weeks in `kv_store`; failures do not advance the ledger, restarts do not duplicate a successful week, and asynchronous webhook completion reports both success and failure so a failed same-week delivery can retry. In-flight week state suppresses duplicate submissions, and stale callbacks cannot alter a newer week's state. Individual renewal, expiry, and orphan sends use expiring DB claims acquired/renewed immediately before each network send, so overlapping processes and partial retries skip successful recipient/channel deliveries. SMTP refusal mappings are committed per recipient and only refused recipients retry; SMTP remains at-least-once across a crash after relay acceptance because its send cannot be atomic with ledger completion. **Orphan notice (Plan 050):** `send_renewal_digest()` also calls `send_orphan_notice()` each run — it emails `permission_tier=='admin'` users (`_admin_emails()`) a flagged list of orphaned certs (from `alerts.find_orphan_certs`), independent of renewal activity so a quiet week still surfaces them. No orphans or no admins ⇒ nothing sent; logs its own SMTP failures and does not gate the digest's return value.

...
- **Plan 049** (maintenance entry): **proposed 2026-06-11** — the last development plan before maintenance mode. Phased: P0 decide (AD CS spike WI-D.1 + positioning window thesis), P1 validate Plan 048 analytics against the real estate, P2 entry-fee debt (WI-031 settings decomposition, WI-025 bare-excepts in alerting/scheduler/scan first, WI-026 SSRF integration tests, settings-POST + /readiness e2e), P3 boring ops (real-DB migration test, restore runbook exercise, SC-081 date-flip tests, dependency cadence), P4 the maintenance contract in AGENTS.md. **P0 before P2** — its outcome can change the rest. See `plans/049-maintenance-entry.md`.
...
- **`digest.py`** — Weekly renewal digest (Plan 048 WI-3.1). `build_renewal_digest()` queries event_log for `cert_renewed` and `renewal_overdue` events, groups by owner. Zero-activity periods produce nothing.

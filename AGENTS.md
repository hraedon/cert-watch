...
- **BC-083: Secure-by-default.** When no `AUTH_PROVIDER` is configured and `CERT_WATCH_ALLOW_UNAUTH` is unset, the app **refuses to start** on a non-loopback bind address (`SystemExit` with remediation message). Loopback binds (`127.0.0.1`, `::1`, `localhost`) are always exempt. `CERT_WATCH_ALLOW_UNAUTH=1` restores the old open-by-default behavior. The `needs_setup` flag no longer requires `host_count == 0` — an existing instance without auth will still force the setup wizard. **BC-090:** the check reads the bind from `CERT_WATCH_HOST`, and the `cert-watch` entrypoint (`__main__`) is the single source of truth — it accepts `--host`/`--port` and normalizes `CERT_WATCH_HOST` to whatever it passes to uvicorn, so the check can't diverge from the real bind. All deploys launch through the entrypoint (IIS HttpPlatformHandler runs `-m cert_watch --host 127.0.0.1`); a raw `uvicorn cert_watch.app:app` that bypasses it must set `CERT_WATCH_HOST`/`CERT_WATCH_ALLOW_UNAUTH` or it fails closed.
...
- **CSP `script-src` allows `'unsafe-inline'`** (BC-075) — for now. The nonce *plumbing* is back (`CSPNonceMiddleware`, pure ASGI, issues a per-request nonce into `scope['state']`; templates read it via `{{ request.state.csp_nonce }}`), but `_build_csp(nonce)` still emits `'unsafe-inline'` and ignores the nonce, because some templates retain inline `on*=` handlers and a nonce would make browsers ignore `'unsafe-inline'` and break them (the Plan 020 S4 regression). The **flip is a one-liner** in `_build_csp` once handlers hit zero. A ratchet test (`tests/test_no_inline_handlers.py`) enforces no *new* inline handlers — current budget: `dashboard.html` 16, `certificate_detail.html` 4, `settings.html` 3 (23 left); `base.html` is the converted reference (0). `style-src` keeps `'unsafe-inline'` regardless (inline `style=` custom properties). XSS mitigations today are `escHtml()` + the urgency-class whitelist. `security_headers_middleware` sets CSP + `X-Content-Type-Options: nosniff` + `X-Frame-Options: DENY`.
...
Unresolved backlog (as of 2026-06-02): **BC-071** (OAuth userinfo nonce binding — partial), **BC-073** (grouped dashboard full-load — partial, low), **BC-075** (CSP `script-src` `'unsafe-inline'` — nonce plumbing + no-inline-handler guardrail landed; 23 handlers left to convert, then flip; see the CSP convention above), **BC-082** (kv_store plaintext secrets at rest). New/unstarted: **BC-020** (host-level notes — feature, medium), **BC-021** (progressive enhancement for dashboard — improvement, low), **BC-093** (enforce mypy strict in CI — improvement, low), **BC-094** (decompose queries.py by concern — improvement, low), **BC-095** (deduplicate SSRF pre-check logic — improvement, low). Resolved: BC-081 (session revocation via per-user version counter), BC-083 (secure-by-default auth), BC-084 (A+ non-443 posture), BC-085 (3.13 container), BC-086 (read-only vs read-write role tier), BC-087 (webhook/test missing write gate), BC-088 (create alert-group ignored webhook URL validation), BC-089 (CSV export duplicate columns), BC-090 (BC-083 check regressed IIS startup; entrypoint now the single source of truth for the bind host), BC-091 (mypy strict — dup of BC-093), BC-092 (queries.py decomposition — dup of BC-094). Resolved breadcrumbs live in the DB as history.

...
2. **Read the scaffold.** `src/cert_watch/` — `app.py` (FastAPI app factory + lifespan), `routes/` (HTTP handlers), `middleware.py` (security middleware + FastAPI deps), `templates/`, `static/`, plus feature modules: `certificate_model.py`, `cert_chain.py`, `scan.py`, `upload.py`, `alerts.py`, `scheduler.py`, `posture.py`, `config.py`, `attention.py` (home-page attention-queue assembly). The `auth/` package and `database/` package hold the two largest concerns (see Architecture notes). **IA note (2026-08-30, branch `redesign/attention-home`):** `/` is Home (attention queue + expiry horizon), the inventory table lives at `/browse`; legacy `/` query-param bookmarks 307-redirect to `/browse`. Notes are host-scoped only (migration 0031 merges `certificates.notes`, retaining the deprecated column only for unmatched notes; see UI-INVENTORY V1/V2).

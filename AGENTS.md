...
Unresolved backlog (as of 2026-06-03): **BC-020** (host-level notes — feature, medium), **BC-021** (progressive enhancement for dashboard — improvement, low), **BC-093** (enforce mypy strict in CI — improvement, low), **BC-094** (decompose queries.py by concern — improvement, low), **BC-095** (deduplicate SSRF pre-check logic — improvement, low), **BC-096** (decouple test monkeypatching from route-level import names — improvement, low), **BC-097** (Discover page sync CT reconciliation perf — improvement, medium), **BC-099** (mis-issuance detection stubbed out — feature, medium), **BC-100** (fleet posture hardcoded CA detection — improvement, low), **BC-106** (integration test mock signatures stale — bug, medium), **BC-107** (asyncio.run loop conflicts in tests — bug, medium), **BC-108** (surface chain_incomplete in UI — feature, medium), **BC-109** (cert_scan_errors_total counter — feature, low), **BC-110** (split /healthz into /readyz — feature, low), **BC-111** (migrate /metrics to prometheus_client — feature, low), **BC-112** (test for _rate_db_initialized flag — bug, low), **BC-103** (webhook alert presets — feature, low), **BC-104** (REST API machine-to-machine auth — feature, medium). Resolved: BC-071, BC-073, BC-075, BC-081, BC-082, BC-083, BC-084, BC-085, BC-086, BC-087, BC-088, BC-089, BC-090, BC-091, BC-092, BC-098 (detail page panel cards — fixed by wave-2 restructure), BC-101 (trend charts monthly bucketing — pivot helpers + dynamic bar scaling in routes/views.py), BC-102 (in-UI local admin password rotation — POST /settings/change-password, rotation banner, change-password form in settings auth tab), BC-115 (LDAP StartTLS bind fix), BC-116 (webhook SSRF redirect validation), BC-117 (OCSP/CRL SSRF validation), BC-118 (configurable LDAP group filter). Resolved breadcrumbs live in the DB as history.

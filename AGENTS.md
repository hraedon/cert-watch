...
Unresolved backlog (as of 2026-06-02): **BC-031** (PostgreSQL/MSSQL backend — deferred), **BC-071** (OAuth userinfo nonce binding — partial), **BC-073** (grouped dashboard full-load — partial, low), **BC-075** (CSP `script-src` `'unsafe-inline'`; deferred to design rewrite), **BC-082** (kv_store plaintext secrets at rest). Resolved: BC-081 (session revocation via per-user version counter), BC-083 (secure-by-default auth), BC-084 (A+ non-443 posture), BC-085 (3.13 container), BC-086 (read-only vs read-write role tier), BC-087 (webhook/test missing write gate), BC-088 (create alert-group ignored webhook URL validation), BC-089 (CSV export duplicate columns). Resolved breadcrumbs live in the DB as history.

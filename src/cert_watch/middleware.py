...
def get_auth_context(request: Request) -> dict:
    """Return template context with auth_user, may_write flag, and local_admin_autogenerated.
    ...
    ``local_admin_autogenerated`` is True when the local admin password has
    not been rotated from the auto-provisioned first-run credential (BC-102).
    """
    ...
    if settings:
        ...
        autogenerated = (
            bool(kv_get(settings.db_path, "local_admin_user"))
            and kv_get(settings.db_path, "local_admin_autogenerated") == "1"
        )
    return {
        "auth_user": username,

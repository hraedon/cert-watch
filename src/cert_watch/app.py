...
from cert_watch.auth.guards import install_guard_handler
...
from cert_watch.routes.not_found import install_not_found_handler
...
def _provision_initial_admin(s: Settings, *, security: SecurityContext) -> None:
    ...
    from cert_watch.database.kv_store import kv_set_multi, kv_set_secret

    username = "admin"
    password = _secrets.token_urlsafe(18)
    pw_hash = _scrypt_hash(password)

    # Write the password file BEFORE persisting to kv_store so a failed write
    # leaves no half-provisioned admin (WI-090). If the file can't be written,
    # return without committing — the lifespan's fail-closed path (SystemExit)
    # refuses to serve open on a network-exposed bind.
    ...
    try:
        ...
        logger.error(
            "could not write %s — admin NOT provisioned. The app will refuse "
            "to start (fail-closed). To recover: fix the data directory "
            "permissions and restart, or set CERT_WATCH_LOCAL_ADMIN_PASSWORD_HASH "
            "via 'cert-watch hash-password'.",
            pw_file,
        )
        return

    # H3: encrypt the password hash at rest via kv_set_secret so a DB
    # backup/dump does not expose the scrypt hash for offline cracking.
    from cert_watch.database import derive_encryption_key

    enc_key = derive_encryption_key(security.signing_key)
    kv_set_multi(s.db_path, {
        LOCAL_ADMIN_USER: username,
        LOCAL_ADMIN_AUTOGENERATED: "1",
        SETUP_COMPLETE: "1",
    })
    kv_set_secret(s.db_path, LOCAL_ADMIN_PASSWORD_HASH, pw_hash, enc_key)

    logger.warning(
        "No authentication was configured; created an initial admin '%s' so the "
        "app does not serve open on a network. One-time password written to %s "
        "(chmod 600). Log in, then configure AUTH_PROVIDER (LDAP/OAuth) or pin "
        "a password via CERT_WATCH_LOCAL_ADMIN_PASSWORD_HASH, and delete the "
        "password file. Set CERT_WATCH_ALLOW_UNAUTH=1 to intentionally run open.",
        username, pw_file,
    )
...
def create_app(
    *,
    ...
    install_not_found_handler(application)

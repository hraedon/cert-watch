...
from cert_watch.database import bump_session_version, kv_get, kv_set, kv_set_secret
...
async def change_local_admin_password(request: Request) -> RedirectResponse:
    ...
    if enc_key:
        kv_set_secret(db, LOCAL_ADMIN_PASSWORD_HASH, _scrypt_hash(new_password), enc_key)
    else:

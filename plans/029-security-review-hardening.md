...
### E. Login CSRF (#19) — Low
`POST /login` is a public path (no session yet), so the double-submit check is
skipped by design; SameSite=strict blocks cross-site, but a hostile same-site
tab / open redirector could log a victim into an attacker's account. Fix: a
per-IP login nonce issued by `GET /login` and echoed in the POST. Low severity
for this app; weigh against the added flow complexity.

### F. scrypt parameters (#11) — Low
`verify_scrypt_hash` only *warns* below `n=2**14` then proceeds
(`auth/local_admin.py:57`). **Do not** simply reject below the floor — that
locks out an operator whose stored hash is weak. Phased: (1) flag weak hashes in
the UI / startup check and prompt re-hash; (2) once migration is in place, refuse
weak hashes at verify. **Default cost:** M3 wants `2**17`; note that scrypt at
`n=2**17, r=8` needs ~134 MB **per hash**, a real DoS/resource surface on small
containers — benchmark before bumping past `2**15`/`2**16`. Decide deliberately.
...
3. **CI/supply-chain batch** — independent, high leverage, low risk; can land
   anytime.
4. **E (login CSRF)**, **F (scrypt)** — deliberate design calls; don't rush F's
   default bump.

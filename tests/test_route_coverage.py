...
def test_favicon_ico_redirects_to_svg(reload_app):
    app_mod = reload_app()
    with TestClient(app_mod.app) as client:
...
def test_favicon_svg_served(reload_app):
    app_mod = reload_app()
    with TestClient(app_mod.app) as client:
...
def test_host_detail_failure_pill(reload_app, tmp_path):
    app_mod = reload_app()
    db = tmp_path / "cert-watch.sqlite3"
    ...
    host_id = host_repo.add("fail.example.com", 443)
    record_scan_history(
        db,
        ScanHistory(

...
# ── #113 item 7: report formatting, one status rule, old exports ──────────────

_V102_FIXTURE = Path(__file__).parent / "fixtures" / "compliance_report_v1_0_2.json"
# Not a secret: the key the fixture was signed with when it was generated by
# the 1.0.2 code (tests/fixtures/compliance_report_v1_0_2.json).
_V102_KEY = "fixture-key-not-a-secret"


class TestReportIssue113:
    def test_report_exported_by_1_0_2_still_verifies(self):
        """A report signed before #113 changed the report's content must still
        verify: verification rebuilds from the file's own signed values, and
        the #113 changes altered values, not the report's schema."""
        data = json.loads(_V102_FIXTURE.read_text())
        # The fixture really is the old output: doubled-port source data and
        # the expiry-only status for a 30-day certificate.
        hosts = [e["host"] for b in data["remediation_buckets"] for e in b["entries"]]
        assert "ldap.example.test:636" in hosts
        assert verify_report_signature(data, _V102_KEY) == (True, "PASS")

    def test_report_exported_by_1_0_2_still_detects_tampering(self):
        data = json.loads(_V102_FIXTURE.read_text())
        entry = data["remediation_buckets"][1]["entries"][0]
        entry["days_remaining"] = 300
        ok, _ = verify_report_signature(data, _V102_KEY)
        assert ok is False

    def test_cli_pass_line_says_what_passed(self, tmp_path, monkeypatch, capsys):
        monkeypatch.setenv("CERT_WATCH_DATA_DIR", str(tmp_path))
        monkeypatch.setenv("CERT_WATCH_AUTH_SECRET", _V102_KEY)
        code, out = _cli_verify(tmp_path, json.loads(_V102_FIXTURE.read_text()), capsys)
        assert code == 0
        first = out.splitlines()[0]
        assert first == "PASS — signature and content hash verified"
        assert "PASS — PASS" not in out

    def test_html_report_shows_each_endpoint_once(self, tmp_path, reload_app):
        from datetime import UTC, datetime, timedelta
        ...
        db = tmp_path / "cert-watch.sqlite3"
        now = datetime.now(UTC)
        seed_host(db, "ldap.example.test", 636)
        for cid, host, port, source in (
            ("c-ldap", "ldap.example.test", 636, "scanned"),
            ("c-file", None, 0, "uploaded"),
        ):
            seed_certificate(
                db,
                Certificate(
                    subject=f"CN={host or 'file.example.test'}", issuer="CN=CA",
                    not_before=now - timedelta(days=30),
                    not_after=now + timedelta(days=3, hours=12),
                    fingerprint_sha256=f"fp-{cid}", raw_der=b"\x00",
                ),
                cert_id=cid, hostname=host, port=port, source=source,
            )
        with TestClient(app_mod.app) as client:
            html = client.get("/reports/compliance").text
        assert "ldap.example.test:636:636" not in html
        assert "ldap.example.test:636<" in html
        assert "(uploaded):0" not in html
    ...
    def test_bucket_edges_follow_the_status_thresholds(self, tmp_path, monkeypatch):
        ...
        now = datetime.now(UTC)
        for name, days in (("seven", 7), ("thirty", 30)):
            seed_host(db, f"{name}.example.test", 443)
            seed_certificate(
                db,
                Certificate(
                    subject=f"CN={name}.example.test", issuer="CN=CA",
                    not_before=now - timedelta(days=30),
                    not_after=now + timedelta(days=days, hours=12),
                    fingerprint_sha256=f"fp-{name}", raw_der=b"\x00",
                ),
        ...
        assert urgency["CN=seven.example.test"] == "warning"
        assert urgency["CN=thirty.example.test"] == "healthy"
